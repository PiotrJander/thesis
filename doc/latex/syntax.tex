...
\AF{\_$\,\dot{\to}\,$\_} and \AF{\_$\dot{\times}$\_} respectively, silently threading the
...
Lastly, \AF{$\kappa$} is the constant combinator, ignoring the index.

We make \AF{$\,\dot{\to}\,$} associate to the right as one would expect and give it the
...
\AF{[} \AF{suc} \AF{⊢} (\AB{P} \AF{$\dot{\times}$} \AB{Q}) \AF{$\,\dot{\to}\,$} \AB{R} \AF{]}
desugars to the more verbose type
\AS{∀} \{\AB{i}\} \AS{$\to$} (\AB{P} (\AF{suc} \AB{i}) \AF{×} \AB{Q} (\AF{suc} \AB{i})) \AS{$\to$} \AB{R} \AB{i}.
...
\begin{center}
  \AIC{z} : {\AS{∀} \AB{i} \AB{xs} \AS{$\to$} \AD{Var} \AB{i} (\AB{i} \AIC{:\!:} \AB{xs})}
  \qquad
  \AIC{s} : {\AS{∀} \AB{i} \AB{j} \AB{xs} $\to$ \AD{Var} \AB{i} \AB{xs} \AS{$\to$} \AD{Var} \AB{i} (\AB{j} \AIC{:\!:} \AB{xs})}
\end{center}
...
left abstract here: \AF{$\llbracket$V$\rrbracket$} and \AF{extend}. Their implementations are distinct
for \AF{ren} and \AF{sub} but they serve the same purpose: \AF{$\llbracket$V$\rrbracket$} is used to
...
{(\AB{$\Gamma$} \AR{--Env}) \AB{$\mathcal{V}$} \AB{$\Delta$}} that describe how to associate a value \AB{$\mathcal{V}$}
(variables for renaming, terms for substitution) well scoped and typed in \AB{$\Delta$}
to every entry in \AB{$\Gamma$}. Environments are defined as the following record
structure (using a record helps Agda's type inference reconstruct the type of
values \AB{$\mathcal{V}$} for us):
...
(\AF{□} (\AB{$\mathcal{V}$} \AB{$\sigma$} \AF{$\to$} \AB{$\mathcal{C}$} \AB{$\tau$})).
...
$\Set{}^I$. It has three constructors: \AIC{`$\sigma$} to store data (the rest of
...
The recursive function \AF{$\llbracket$\_$\rrbracket$} makes the interpretation of the
...
generic programs for whole classes of data types. The decoding function \AF{$\llbracket$\_$\rrbracket$}
...
\AIC{`$\blacksquare$} constructors are as in the CDMM \AD{Desc} type, and are used to
...
The meaning function \AF{$\llbracket$\_$\rrbracket$} we associate to a description follows closely
...
to interpret {\AIC{`X} \AB{$\Delta$} \AB{j}} (i.e. substructures of sort \AB{j} with
newly-bound variables in \AB{$\Delta$}) in an ambient scope \AB{$\Gamma$} as {\AB{X} \AB{$\Delta$} \AB{j} \AB{$\Gamma$}}.
...
The astute reader may have noticed that \AF{$\llbracket$\_$\rrbracket$} is uniform in $X$ and $\Gamma$; however
refactoring \AF{$\llbracket$\_$\rrbracket$} to use the partially applied $X\,\_\,\_\,\Gamma$ following
...
If we pre-compose the meaning function \AF{$\llbracket$\_$\rrbracket$} with a notion of `de Bruijn scopes'
(denoted \AF{Scope} here) which turns any \AB{I} \AF{--Scoped} family into a function
of type \AD{List} \AB{I} \AS{$\to$} \AB{I} \AF{--Scoped} by appending the two
...
by two \AB{I}\AF{--Scoped} families \AB{$\mathcal{V}$} and \AB{$\mathcal{C}$} corresponding
...
the type \AF{List} \AB{I} \AS{$\to$} \AB{I}\AF{--Scoped}, i.e. that gets access
...
with a fundamental lemma: all \AB{I} \AF{--Scoped} families \AB{$\mathcal{V}$} and
\AB{$\mathcal{C}$} satisfying the three criteria we have put forward give rise
...
(thanks to a auxiliary function \AF{\_$\mathbin{>\!\!>}$\_} which given two environments
{(\AB{$\Gamma$} \AR{--Env}) \AB{$\mathcal{V}$} \AB{$\Theta$}} and {(\AB{$\Delta$} \AR{--Env}) \AB{$\mathcal{V}$} \AB{$\Theta$}}
produces an environment {((\AB{$\Gamma$} \AF{$+\!+$} \AB{$\Delta$}) \AR{--Env}) \AB{$\mathcal{V}$} \AB{$\Theta$})}.
...
For any \AR{VarLike} \AB{$\mathcal{V}$}, we can define \AF{fresh\textsuperscript{r}} of
type {(\AB{$\Gamma$} \AR{--Env}) \AB{$\mathcal{V}$} (\AB{$\Delta$} \AF{$+\!+$} \AB{$\Gamma$})} and \AF{fresh\textsuperscript{l}} of
type {(\AB{$\Gamma$} \AR{--Env}) \AB{$\mathcal{V}$} (\AB{$\Gamma$} \AF{$+\!+$} \AB{$\Delta$})} by combining the use
...
constructor which we have renamed \AIC{$\curvearrowleft{}$} (pronounced ``backpointer'')
to match the domain-specific meaning.
We can see this approach in action in the examples
\AF{[0, 1]} and \AF{01$\circlearrowleft{}$} (pronounced ``0-1-cycle'') which describe
...
corresponds to the example \AF{01$\circlearrowleft{}$} given above. The definition
...
the definition of \AR{∞Tm} was made easy by reusing \AF{$\llbracket$\_$\rrbracket$}.
...
\begin{itemize}
  \item In the stop token case \AIC{`$\blacksquare$} \AB{i}, the two layers are considered to
...
\AF{$\llbracket$} \AB{d} \AF{$\rrbracket$} (\AF{Kripke} \AB{$\mathcal{V}_1$} \AB{$\mathcal{C}_1$}) and
\AF{$\llbracket$} \AB{d} \AF{$\rrbracket$} (\AF{Kripke} \AB{$\mathcal{V}_2$} \AB{$\mathcal{C}_2$}) are
...
The definition of fusion is parametrised by three relations: \AB{$\mathcal{R}$\textsuperscript{E}} relates
triples of environments of values in {(\AB{$\Gamma$} \AR{--Env}) \AB{$\mathcal{V}$$_1$} \AB{$\Delta$}},
{(\AB{$\Delta$} \AR{--Env}) \AB{$\mathcal{V}$$_2$} \AB{$\Theta$}} and {(\AB{$\Gamma$} \AR{--Env}) \AB{$\mathcal{V}$$_3$} \AB{$\Theta$}}
respectively; \AB{$\mathcal{R}^{\mathcal{V}}$} relates pairs of values \AB{$\mathcal{V}$$_2$} and \AB{$\mathcal{V}$$_3$};
and \AB{$\mathcal{R}^{\mathcal{C}}$}, our notion of equivalence for evaluation results, relates pairs
of computation in \AB{$\mathcal{C}$$_2$} and \AB{$\mathcal{C}$$_3$}.
...
term from a \AB{$\mathcal{C}$$_1$}. Hence the first constraint: the existence of a \ARF{quote\textsubscript{1}}
...
\AB{$\rho_1$} in {(\AB{$\Gamma$} \AR{--Env}) \AB{$\mathcal{V}$$_1$} \AB{$\Delta$}}, \AB{$\rho_2$} in
{(\AB{$\Delta$} \AR{--Env}) \AB{$\mathcal{V}$$_2$} \AB{$\Theta$}}, and \AB{$\rho_3$} in {(\AB{$\Gamma$} \AR{--Env}) \AB{$\mathcal{V}$$_3$} \AB{$\Theta$}}
are related by \AB{$\mathcal{R}$\textsuperscript{E}} and we are given a thinning \AB{$\sigma$} from \AB{$\Theta$} to \AB{$\Omega$}
...
Remembering that \AF{\_$\mathbin{>\!\!>}$\_} is used in the definition of \AF{body} (Figure~\ref{def:body}) to
combine two disjoint environments {(\AB{$\Gamma$} \AR{--Env}) \AB{$\mathcal{V}$} \AB{$\Theta$}} and
{(\AB{$\Delta$} \AR{--Env}) \AB{$\mathcal{V}$} \AB{$\Theta$}} into one of type
{((\AB{$\Gamma$} \AF{$+\!+$} \AB{$\Delta$}) \AR{--Env}) \AB{$\mathcal{V}$} \AB{$\Theta$})}, we mechanically need a
constraint stating that \AF{\_$\mathbin{>\!\!>}$\_} is compatible with \AB{$\mathcal{R}$\textsuperscript{E}}. We demand
...
Provided that all three evaluation environments are related by \AB{$\mathcal{R}$\textsuperscript{E}} this should
...
\AF{Kripke} functional spaces are reified by being fed placeholder \AB{$\mathcal{V}$$_1$} arguments
(which can be manufactured thanks to the \ARF{vl\textsuperscript{$\mathcal{V}$$_1$}} we mentioned before) and
...
of \AF{01$\circlearrowleft{}$} defined in Section~\ref{def:colist} is indeed bisimilar to \AF{01⋯}
...
%\caption{The unfolding of \AF{01$\circlearrowleft{}$} is bisimilar to the direct-style \AF{01⋯}}
%\end{figure}
\end{center}
