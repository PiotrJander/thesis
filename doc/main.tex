...
\begin{definition}
  ...
  \item (\textit{Abstraction}) If \AS{N \tis T.subst (T.rename (pack
      s) <\$> E ∙ T.V z) N†}, then \AS{S.L N \tis T.L N† E}.
...
\begin{definition}
  ...
  we say that \AS{≈} is a \textbf{simulation} if and only if for all
  terms \AS{M} and \AS{N} in \AS{A}, and \AS{M†} in \AS{B}, if \AS{M}
  reduces in a single step to \AS{N}, then there exists a term \AS{N†}
  in \AS{B} such that \AS{M†} reduces to \AS{N†} in a single step, and
  \AS{N} is in the \AS{≈} relation with \AS{N†}: \AS{N ≈ N†}.
\end{definition}
