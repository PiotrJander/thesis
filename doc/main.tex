...
instantiation of the fusion lemma about a composition of renaming and
substitution. 
...
\begin{lemma}
  ...
  and \AS{ρ†} are two substitutions which take variables \AS{x} in
  \AS{Γ} to terms in \AS{Δ}, such that for all \AS{x} we have that
  \AS{lookup ρ x \ti lookup ρ† x}. Then given compatible terms \AS{M
    \ti M†} in \AS{Γ}, the results of applying \AS{ρ} to \AS{M} and
  \AS{ρ†} to \AS{M†} are also compatible: \AS{S.subst ρ M \ti T.subst
    ρ† M†}.
\end{lemma}
