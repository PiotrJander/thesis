...
We have showed that the compability relation is a bisimulation. But to
link it to closure conversion and show that our implementation
\AS{\_†} is correct, we need to show that the graph relation of
\AS{\_†} is contained in the compatibility relation. In other words,
if \AS{M} is a source term, then we have \AS{M \tis M †}, where \AS{M
  †} is the translation of \AS{M} with \AS{\_†}. Thus, we need to show:
...
However, as we argued, the graph relation of any well-behaved closure
conversion function is contained within the compatibility relation. It
is instructive to see that this is the case for a simpler version of
closure convertion, namely, a trivial closure conversion algorithm
...
compatibility relation. The proof is by straightforward induction; in
the abstraction case, we need to argue that applying an identity
