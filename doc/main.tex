...
\begin{definition}
  ...
  \item (\textit{Variable}) For any given variable (proof of context
    membership) \AS{x}, we have \AS{S.` x \ti T.` x}.

  \item (\textit{Application}) If \AS{M \ti M†} and \AS{N \ti N†},
    then \AS{M · N \ti M† · N†}.

  \item (\textit{Abstraction}) If \AS{N \ti subst (exts E) N†},
    then \AS{S.L N \ti T.L N† E}.
...
What are the necessary conditions for \AS{S.L N \ti T.L N† E}, where
\AS{S.L N} and \AS{T.L N† E} are defined in a context \AS{Γ}? Clearly,
we cannot require that \AS{N \ti N†}, as the context \AS{Δ} in which
...
idenitity substitutions \AS{id-subst}):
...
\subsubsection{Minimising closure conversion is contained in the
  similarity relation}
\label{sec:minim-clos-conv-1}

TODO

With the notion of similarity formalised, we are in a position to
define bisimulation.
