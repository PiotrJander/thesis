...
Closure conversion is a compilation phase present in every compiler
which takes a language with first-class, nested functions to machine
code. Suppose the source language is simply typed lambda calculus:

\AS{S ∷= x | S S | λx.S }

Then the target language should contain closures in place of
abstractions:

\AS{T ∷= x | T T | ⟪ λx.λe.T , E ⟫ }

In the language with closures, a function takes an argument and an
environment, which are the only values it can use in its
body. Functions can be partially applied to environments, forming
closures.
...
the transformation is correct according to some definition of
...
Futhermore, the correctness lemma depends on multiple smaller results
about the interplay between renaming, substitution, the translation
function, and others.
...
A \textbf{contribution} of this project, other than achieving the
original objectives, is to demonstrate that languagues with closures
and closure conversion are problematic for current state-of-the-art
techniques for mechanising language meta-theory.
...
Recall that the \textbf{original objectives} of the project were [TODO
recall or just refer to the intro?]:
...
generic proving techniques, e.g. from AACMM, to try to reduce the
burden of repeating the same effort for each new laguage. However,
this project's insights seem to imply that closure conversion does not
fit into the framework provided by AACMM, and that further
developments in generic proving would be needed to support closure
conversion.
