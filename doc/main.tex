...
\section{Program derivation in compilers and its limitations}
...
rules. Note that GHC makes no attempt that the rules preserves the
meaning, or that rewriting would terminate: a programmer could
externally ensure these properties, e.g. using a proof assistant.
...
which would require human insight. The next section, which discusses
some of the rewrite steps from the UG4 project, has examples of rules
which were invented for a specific derivation, and it is difficult to
conceive that they could all be provided to the compiler in advance.
...
The next section does exactly that: it expresses a part of the
derivation from the UG4 project in Agda.
