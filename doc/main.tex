...
\abstract{ There is an ongoing effort in the programming languages
  ...
  requires repeating considerable meta-theoretical boilerplate for
  each intermediate language used by the compiler.
  ...
  This work builds on a line of research which gave rise to a
...
verification, or formally proving that a program conforms to a
...
the compiled machine code which is actually executed. To bridge the
...
binders will allow us to prove \textit{fusion lemmas} in [section]
without using the machinery of ACMM, which will significantly simplify
the proofs.
