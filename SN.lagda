...
σ∘σ≡σ : ∀ {Γ Δ Θ}
      ...
      → Substitution _⊢_ Δ Θ
        ----------------
      → Substitution _⊢_ Γ Θ
σ∘σ≡σ γ ρ N = subst ρ (γ N)

\end{code}

We'd like to show that (subst (exts γ) e) [ v ] ≡ subst γ' e where γ' = γ[#0 → v].
We could show that substitutions are closed under composition.
